...
You will probably mostly need `process` and `check` so it would be good to start
reading the docs for these. Skim over the rest and come back when you need it.
...
``` clojure
$ clojure -Sdeps '{:deps {babashka/process {:mvn/version "0.2.9"}}}'
